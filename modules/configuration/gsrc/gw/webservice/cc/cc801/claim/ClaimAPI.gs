package gw.webservice.cc.cc801.claim
...
uses gw.webservice.cc.cc801.CCWsiEntityApiBase
uses gw.webservice.cc.cc801.dto.ActivityDTO
uses gw.webservice.cc.cc801.dto.ActivityPatternDTO
uses gw.webservice.cc.cc801.dto.ClaimDTO
uses gw.webservice.cc.cc801.dto.ClaimInfoDTO
uses gw.webservice.cc.cc801.dto.ContactDTO
uses gw.webservice.cc.cc801.dto.DocumentDTO
uses gw.webservice.cc.cc801.dto.ExposureDTO
uses gw.webservice.cc.cc801.dto.NoteDTO
...
@WsiWebService("http://guidewire.com/cc/ws/gw/webservice/cc/cc801/claim/ClaimAPI")
...
class ClaimAPI extends CCWsiEntityApiBase {
  ...
  function completeActivity(activityID : String) {
    Transaction.runWithNewBundle(\ bundle -> {
      var act = fetchByID<Activity>(activityID, bundle)
      act.complete()
    })
  }
  ...
  public function createActivity(dto : ActivityDTO) : String {
    ...
    ArgCheck.nonBlank(dto.ClaimID,           "activityDTO.ClaimID")

    var activity : Activity
    Transaction.runWithNewBundle(\ bundle -> {
      var claim = bundle.add(dto.Claim)
      activity = assembleFromDTOs({dto}.toTypedArray(), claim, dto.Exposure).first()
    })
  ...
  function getActivityPatternData(publicID : String) : ActivityPatternDTO {
    return ActivityPatternDTO.valueOf(fetchByID(publicID))
  }
  ...
  function getDtoForClaim(claimID : String) : ClaimDTO {
    return ClaimDTO.valueOf(fetchByID(claimID))
  }
  ...
  function addFNOL(claimDTO : ClaimDTO, policyDTO : PolicyDTO) : String {
    ...
    Transaction.runWithNewBundle(\ bundle -> {
      ...
      Delegate.addFNOL(fnolClaim, false)

      if (claimDTO.AssignedUserID == null) {
        if (claimDTO.AssignedGroupID != null) {
          fnolClaim.assignGroup(claimDTO.AssignedGroup)
        }
      }
      else {
        if (claimDTO.AssignedGroupID == null) {
          fnolClaim.assignUserAndDefaultGroup(claimDTO.AssignedUser)
        }
        else {
          fnolClaim.assign(claimDTO.AssignedGroup, claimDTO.AssignedUser)
        }
      }
    })
  ...
  private function instantiateMapper(type:IType) : FNOLMapper {
    if(not FNOLMapper.Type.isAssignableFrom(type)) {
  ...
  function doesExist(claimID : String) : boolean {
    ...
    try {
      var claim = fetchByID<Claim>(claimID)
  ...
  function claimsExist(claimNumbers : String[]) : String[] {
    var claimQ = new Query(Claim)
        .compareIn(Claim#ClaimNumber, claimNumbers)
        .select()
    var results = claimQ.map(\ clm -> (clm as Claim).ClaimNumber)
  ...
  function checkValid(claimID : String, validationLevel : ValidationLevel) : boolean {
    ...
    Transaction.runWithNewBundle(\ bundle -> {
      var claim = fetchByID<Claim>(claimID, bundle)
   ...
   */
  @Deprecated("Use gw.webservice.cc.cc801.claim.ClaimAPI#startClaimValidation(java.lang.String) instead")
  ...
  function getClaimState(claimID : String) : ClaimState {
    return fetchByID<Claim>(claimID).State
  }
  ...
  function giveUserPermissionsOnClaim(claimID : String, userID : String, accessTypes : ClaimAccessType[]) {
    Transaction.runWithNewBundle(\ bundle -> {
      var claim = fetchByID<Claim>(claimID, bundle)
      var user  = fetchByID<User>(userID, bundle)
  ...
  function getClaimInfo(claimID : String) : ClaimInfoDTO {
    var claim = fetchByID<Claim>(claimID)
    var claimInfo = Claim.finder.findClaimInfoByClaimNumber(claim.PublicID)
    return ClaimInfoDTO.valueOf(claimInfo)
  }
  ...
  */
  ...
  public function createDocument(dto : DocumentDTO) : String {
    ...
    ArgCheck.nonBlank(dto.ClaimID, "documentDTO.ClaimID")
  ...
  function createExposure(dto : ExposureDTO) : String {
    ...
    ArgCheck.nonBlank(dto.ClaimID, "exposureDTO.ClaimID")

    return createExposures(dto.ClaimID, {dto}).first()
  }
  ...
  function getDtoForExposure(exposureID : String) : ExposureDTO {
    return ExposureDTO.valueOf(fetchByID(exposureID))
  }
  ...
  public function getExposureState(exposureID : String) : ExposureState {
    return fetchByID<Exposure>(exposureID).State
  }
  ...
  public function reopenExposure(exposureID : String, reasonType : ExposureReopenedReason, reason : String) {
    Transaction.runWithNewBundle(\ bundle -> {
      var exp = fetchByID<Exposure>(exposureID, bundle)
  ...
  public function createNote(dto : NoteDTO) : String {
    ...
    ArgCheck.nonBlank(dto.ClaimID, "noteDTO.ClaimID")

    var note : Note
    Transaction.runWithNewBundle(\ bundle -> {
      var claim = bundle.add(dto.Claim)
      note = assembleFromDTOs({dto}.toTypedArray(), claim).first()
    })
