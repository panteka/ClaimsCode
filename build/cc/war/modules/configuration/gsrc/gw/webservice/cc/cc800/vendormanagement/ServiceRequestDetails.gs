 ...
 */
@Export
@WsiExportable("http://guidewire.com/cc/ws/gw/webservice/cc/cc800/vendormanagement/ServiceRequestDetails")
final class ServiceRequestDetails {
  ...
  */
   ...
   * service request
   */
  var _messages : List<gw.webservice.cc.cc800.vendormanagement.Message> as Messages
  ...
  construct(serviceRequest : ServiceRequest) {
    ...
    Messages = serviceRequest.Messages.sortByDescending(\ message -> message.SendDate ).map(\ message -> new gw.webservice.cc.cc800.vendormanagement.Message(message)).toList()
