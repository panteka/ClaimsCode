...
class ABContactSystemPlugin implements ContactSystemPlugin {
  ...
  override function hasSyncableChanges(contact: Contact) : boolean {
    return handleErrors(null, contact, \ -> {
      ...
      var graphIter = new ABSyncableGraphIterator(contact as KeyableBean)
        ...
        switch(eventName) {
          case CONTACT_ADDED_EVENT_NAME:
              if(ContactSystemApprovalUtil.shouldCreateInContactSystem(contact)) {
                ...
                var validationResult : wsi.remote.gw.webservice.ab.ab801.abcontactapi.types.complex.ABContactAPIValidateCreateContactResult
                try {
    ...
    if(array != null) {
      for (arrayElemXML in array.XmlBackedInstance) {
        ...
        if(mapper.hasFullRelatedContactField(arrayElemXML)) {
          ...
          var relatedBean = Query.make(Contact).compare("PublicID", Relop.Equals, publicID).select().AtMostOneRow as Contact
          ...
          if(relatedBean.AddressBookUID == null) {
            ...
            var isPending = ContactSystemApprovalUtil.operationRequiresApproval(CONTACT_ADDED_EVENT_NAME, relatedBean)
   ...
   */
  private function getContactFromRemote(remoteXmlInstance:wsi.remote.gw.webservice.ab.ab801.beanmodel.XmlBackedInstance, contact : Contact, bundle : Bundle) : Contact {
    if(contact == null) {
  ...
  private function updateABUIDs(contact:Contact, abuidContainer:AddressBookUIDContainer) {
    ...
    var graphIterator = new ABSyncableGraphIterator(contact as KeyableBean)
