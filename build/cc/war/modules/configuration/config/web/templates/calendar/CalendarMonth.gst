...
<% function renderDayCell(day : com.guidewire.cc.web.calendar.DayModel, showMonthName : boolean, maxEventsToShow : int) { %>
        <div class="<%= getLargeDateLineClass( day ) %>"
            ...
            <%= displaykey.Web.Calendar.Day.Format(showMonthName ? day.format("MMMM d") : day.DayOfMonth) %>
        </div>
             ...
             if (count == maxEventsToShow ) { %>
               <div class="largeDateContentEventLine" style="text-align: center;">
                 <a class="linkDateContentEventLine"
                   ...
                   (<%= day.Events.length-maxEventsToShow + " " + displaykey.JSP.LitigationCalendar.MoreEvents%>)</a>
               </div>
          <%  break;
             }
          %>
...
<% function renderLink( event : com.guidewire.cc.web.calendar.EventModel ) {
     ...
     <% if (event.isCritical(1)) { %><img class="linkDateContentEventCriticalBullet" src="images/app/red_bullet_trimmed.png" title="<%= displaykey.Web.Calendar.Tooltip.TopPriority %>"><% } %>
     <a class="linkDateContentEventLine"
