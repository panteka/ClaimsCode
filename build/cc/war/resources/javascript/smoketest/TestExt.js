...
var Test = function() {
  ...
  function getOptionsInSelect(element) {
    ...
    if(comp instanceof Ext.form.CheckboxGroup) {
      ...
      store.each(function (rec) {
        ...
        option['group'] = rec.get(comp.groupField);
    ...
    if (colIndex < 0) {
      ...
      if (record) {
        Ext.iterate(record.data, function(key, value) {
            var dvColumn;
            if(value && (value.xtype==='templatevaluepanel' || value.xtype==='gfieldset')) {
              dvColumn = value;
            }
  ...
  function setCompValue(comp, value) {
    ...
    if (comp instanceof Ext.form.ComboBox &&
      ...
      gw.ext.Util.setValue(comp, value); // fires 'change' event
    }
  }
  ...
  function getNumCellIteratorEntries(store, record, cellIterFullId) {
    ...
    record.fields.eachKey(function(field) {
      var cellId = gw.GridUtil.getFullIdForCell(store, record, field);
  ...
  function getGrid(lvId, rowOffset, dataIndex) {
    ...
    if(grid){
      ...
      allGrids = Ext.ComponentManager.all.getValues().filter(function(item){
  ...
  function isMenuParentsDisabled(menuItem) {
    if (menuItem instanceof gw.MenuItem || menuItem instanceof Ext.menu.Menu) {
      if (menuItem.disabled) {
        ...
        var parent = (menuItem.parentMenu) ? menuItem.parentMenu : menuItem.ownerItem;
        if (parent) {
...
//            eventObject.initKeyboardEvent(event.substring(2), true, true, null, false, alt, shift, false, keycode, 0);
    ...
    isCompVisible : function(id) {
      ...
      if (comp) {
        ...
        return comp.rendered && comp.getVisibilityEl().isVisible(true) && comp.isVisible(true);
      }
    ...
    getNestedStyleClasses: function(id, asString) {
      ...
      gw.ext.Util.eachDescendent(id, function(descendent, depth) {
    ...
    stripAltValueFromCellText : function(cellElem, cellText) {
      ...
      var footerElem = cellElem.down('.' + gw.ext.Util.getAltValueClass(), true);
      if (footerElem) {
    ...
    getCellAltElem : function (gridId, rowOffset, dataIndex) {
      ...
      return cellElem.down('.' + gw.ext.Util.getAltValueClass(), true);
    },
  ...
  /**
    ...
    setCellValue : function(lvId, rowOffset, dataIndex, value) {
      ...
      if (!column.getEditor()) {
        grid.setRowEditor(editorCfg, rowIndex, dataIndex);
      }
    ...
    setColumnFilterValue : function(lvId, dataIndex, value) {
      ...
      if (filter) {
        if (value) { // set filter value
          var filterForValue = filter.menu.items.find(function(item) {return item.value == value;});
          if (filterForValue) {
    ...
    getColumnFilter: function(grid, dataIndex) {
      ...
      var filter = grid.view.getFeature('gfitlers').getFilter(dataIndex);
    ...
    getColumnFilterValue : function(lvId, dataIndex) {
      ...
      if (filter && filter.active) {
        checkedItem = filter.menu.items.find(function(item) {return item.checked;});
      }
    ...
    showColumnFilterMenuByGrid : function(grid, dataIndex) {
      ...
      headerCt.showMenuBy(columnHeaderTriggerDiv, column);  //@SenchaEnhancement headercontainer.showMenuBy
    ...
    getColumnSortDir : function(lvId, dataIndex) {
      var grid = getGrid(lvId, null, dataIndex);
      var sorter = grid.getStore().sorters.findBy(function(item) {return item.property==dataIndex;});
      return sorter ? sorter.direction : null;
    },

    isColumnGrouped : function(lvId, dataIndex) {
      var grid = getGrid(lvId, null, dataIndex);
      var sorter = grid.getStore().groupers.findBy(function(item) {return item.property==dataIndex;});
      return sorter != null;
    },
    ...
    getColumnFooterElem : function (lvId, rowOffset, dataIndex) {
      ...
      if (summaryFeature && summaryFeature.showSummaryRow) {
        summaryData = grid.store.proxy.reader.jsonData.origSummaryData;
        if(summaryData) {
        ...
        if(footerColumnId){
          ...
          // next footer row
          getFooterDiv = function(enclosingOrPrecedingDiv) {
            var columnIndex = getColumnIndexForColumnId(grid, footerColumnId);
            columnIndex -= getNumberOfInvisibleColumns(grid, columnIndex);
            var footerDiv = enclosingOrPrecedingDiv.down('.x-grid-row-summary .x-grid-cell-first'); //TODO: Card 371: CSS dependency
            if (!footerDiv) {
              footerDiv = enclosingOrPrecedingDiv.next('.x-grid-wrap-row'); //TODO: Card 371: CSS dependency
              footerDiv = footerDiv.down('.x-grid-row-summary .x-grid-cell-first'); //TODO: Card 371: CSS dependency
            }
          ...
          if (grid.getSummaryFeature().id === gw.SimpleGrid.SUMMARY_FEATURE_ID) {
            ...
            if (cellEl) {
              var gridGroup = cellEl.up('.x-grid-wrap-row')|| cellEl.up('.x-grid-data-row'); // the row or group
    ...
    getColumnFooterAltElem : function(gridId, rowOffset, dataIndex) {
      ...
      return footerElem.down('.' + gw.ext.Util.getAltValueClass(), true);
    },
          ...
          return !e.disabled &&
               (e.tagName == 'INPUT' || e.tagName == 'TEXTAREA' || e.tagName == 'SELECT' || e.href != null || e.onclick != null ||
                 ...
                 //TODO: ExtJs upgrade4: check if a tree node is disabled
                    Ext.fly(e).hasCls('x-grid-row') && Ext.fly(e).up('.x-tree-panel', 6) != null /*&& !Ext.fly(e).hasCls('x-tree-node-disabled')*/ || // tree node
    ...
    getOptionHTMLTextInSelect:function (element) {
      ...
      if (comp) {
        ...
        if (pickerId) {
          ...
          if (e) {
            var selectOptions = e.getElementsByTagName('ul')[0].childNodes;
            for (var i = 0; i < selectOptions.length; i++) {
    ...
    */
    ...
    openMenu : function(comp) {
      ...
      if (comp) {
        if (comp.showMenu) {
          ...
          if (comp.expandMenu) {
            comp.activate(); // @SenchaUpgrade non-public
            comp.expandMenu(0); // @SenchaUpgrade non-public
          }
    ...
    getValue : function(e) {
      ...
      if (comp) {
        ...
        } else if (comp.getValue) { // an EXT component with value
          value = gw.ext.Util.getFieldValue(comp);
        } else {
